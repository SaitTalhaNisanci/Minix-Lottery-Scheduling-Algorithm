 ...
 *   do_ticket            Request to change the ticket amount on a proc
 *   do_torpil            Request to change torpil of a process 
...
int DYNAMIC =0; // If 1 then decrease tickets of CPU BOUND processes. Increase the tickets of IO BOUND processes.
...
//If we have some tickets that mean there is a process in the 13th queue.
	if(total_tickets>0){
	  ...
	  winner = rand()%total_tickets +1 ;//+1 to ensure we choose at least one process.
	 for(proc_nr =0, rmp = schedproc; proc_nr <NR_PROCS;proc_nr++,rmp++){
...
int do_start_scheduling(message *m_ptr)
{
	...
	rmp->tickets=10;  //Initial number of tickets
	...
	case SCHEDULING_START:
		...
		rmp->is_sys_proc=1; // Set the process as system process
		break;
...
void balance_queues(void)
{
	...
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		if (rmp->flags & IN_USE) {
			if (rmp->priority > rmp->max_priority && rmp->priority != 13) {

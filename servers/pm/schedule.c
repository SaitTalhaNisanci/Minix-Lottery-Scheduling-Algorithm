 ...
 *				*sched_tickets
 *===========================================================================*/
int sched_tickets(struct mproc *rmp, int tickets)
{
	int rv;
	message m;
	...
	if ((rv = _taskcall(rmp->mp_scheduler, SCHEDULING_SET_TICKETS, &m))!= OK) {
		return rv;
	}
 ...
 *				*sched_torpil
 *===========================================================================*/
int sched_torpil(struct mproc *rmp, int torpil)
{
	int rv;
	message m;
	unsigned maxprio;
	...
	m.m1_i2= rmp->mp_endpoint;
	m.m1_i1= torpil;
	if ((rv = _taskcall(rmp->mp_scheduler, SCHEDULING_SET_TORPIL, &m))) {
		return rv;
	}

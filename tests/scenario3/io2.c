...
int main(int argc, char **argv){
	...
	clock_t begin = clock();
	pid_t pid = getpid();
	fprintf(log,"Number 2 with 14 tickets has started %d pid at %s\n",pid,ctime(&current_time));
	int counter = 50;
//	printf("pid %d: started ",pid);
	...
	printf("Number 2 with 14 tickets:pid %d: finished \n",pid);
	fprintf(log,"Number 2 with 14 tickets pid %d: finished %s\n",pid,ctime(&current_time));
}

 ...
 */
  ...
  int isLooked = 0; // 1 if the cpu looked at the torpil queue,14
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	if(q==12 && isLooked==0){
		
		q=14;//Torpil queue will be chosen before 12 and 13
	}
	if(!(rp = rdy_head[q])) {
		TRACE(VF_PICKPROC, printf("cpu %d queue %d empty\n", cpuid, q););
		if(isLooked==0 && q==14){
			isLooked=1;//Enter here only once
			q =11; //In next iteration it will start from 12th queue	
		}	
		continue;
	}
	if(isLooked==0 && q==14){
